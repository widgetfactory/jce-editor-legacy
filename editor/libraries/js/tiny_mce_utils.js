TinyMCE_Utils = {
    ...
    openBrowser : function(img, input, type, option) {
        if (typeof img == 'string') {
        	img = document.getElementById(img);
        }
    	
    	if (!/mceButtonDisabled/.test(img.className)) {
    		tinyMCEPopup.openBrowser(input, type, option);
    	}
    },
...
function getColorPickerHTML(id, target_form_element) {
    return TinyMCE_Utils.getColorPickerHTML(target_form_element);
}

function updateColor(img_id, form_element_id) {
    return TinyMCE_Utils.updateColor(form_element_id);
    //document.getElementById(img_id).style.backgroundColor = document.forms[0].elements[form_element_id].value;
}

function setBrowserDisabled(id, state) {
    return TinyMCE_Utils.setBrowserDisabled(id, state);
}

function getBrowserHTML(id, target_form_element, type, prefix) {
    return TinyMCE_Utils.getBrowserHTML(id, target_form_element, type, prefix);
}

function openBrowser(img_id, target_form_element, type, option) {
    return TinyMCE_Utils.openBrowser(img_id, target_form_element, type, option);
}

function selectByValue(form_obj, field_name, value, add_custom, ignore_case) {
    if (!form_obj || !form_obj.elements[field_name] || typeof value == 'undefined')
        return;
...
function addClassesToList(list_id, specific_option) {
    TinyMCE_Utils.fillClassList(list_id);
}
...
function convertRGBToHex(col) {
    return $.String.toHex(col);
}

function convertHexToRGB(col) {
    return $.String.toRGB(col);
}
