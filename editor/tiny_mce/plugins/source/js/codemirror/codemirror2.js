...
var CodeMirror = (function() {
    ...
    function CodeMirror(place, givenOptions) {
        ...
        wrapper.innerHTML = '<div style="overflow: hidden; position: relative; width: 1px; height: 0px;">' + // Wraps and hides input textarea
        '<textarea style="position: absolute; width: 2px;" wrap="off"></textarea></div>' + '<div class="CodeMirror-scroll cm-s-' + options.theme + '">' + '<div style="position: relative">' + // Set to the height of the text, causes scrolling
        '<div style="position: absolute; height: 0; width: 0; overflow: hidden;"></div>' + '<div style="position: relative">' + // Moved around its parent to cover visible view
        '<div class="CodeMirror-gutter"><div class="CodeMirror-gutter-text"></div></div>' +
        // Provides positioning relative to (visible) text origin
        '<div class="CodeMirror-lines"><div style="position: relative">' + '<pre class="CodeMirror-cursor">&#160;</pre>' + // Absolutely positioned blinky cursor
        ...
        var inputDiv = wrapper.firstChild,
            ...
            measure = code.firstChild,
            mover = measure.nextSibling,
        ...
        // constructor).
        var mode, lines = [new Line("")],
            work, history = new History(),
            focused;
        loadMode();
        ...
        connect(scroller, "dragenter", function(e) {
            e.stop();
        });
        connect(scroller, "dragover", function(e) {
            e.stop();
        });
        ...
        var instance = {
            ...
            indentLine: operation(function(n) {
                if (isLine(n)) indentLine(n, "smart");
            }),
            ...
            coordsChar: function(coords) {
                ...
                return clipPos({
                    line: line,
                    ch: charFromX(clipLine(line), coords.x - off.left)
                });
            },
            ...
            addWidget: function(pos, node, scroll) {
                ...
                if (scroll) scrollIntoView(pos.x, pos.yBot, pos.x + node.offsetWidth, pos.yBot + node.offsetHeight);
            },
            ...
            getLine: function(line) {
                if (isLine(line)) return lines[line].text;
            },
        ...
        function setValue(code) {
            ...
            updateLines(top, {
                line: lines.length - 1,
                ch: lines[lines.length - 1].text.length
        ...
        function onMouseDown(e) {
            ...
            if (n.parentNode == gutterText) {
                if (options.onGutterClick) options.onGutterClick(instance, indexOf(gutterText.childNodes, n) + showingFrom);
                return e.stop();
            }
        ...
        function onKeyDown(e) {
            ...
            var mod = (mac ? e.e.metaKey : e.e.ctrlKey) && !e.e.altKey,
                anyMod = e.e.ctrlKey || e.e.altKey || e.e.metaKey;
        ...
        function updateLines(from, to, newText, selFrom, selTo) {
            ...
            if (newText.length < 5) highlightLines(from.line, from.line + newText.length)
        }
        ...
        function readInput() {
            ...
            function computeOffset(n, startLine) {
                ...
                for (;;) {
                    ...
                    if (found == -1 || (text.charAt(found - 1) == "\r" ? found - 1 : found) >= n) return {
                        line: startLine,
                        ch: n - pos
                    };
                    ++startLine;
            ...
            if (from.line == to.line && from.line == sel.from.line && from.line == sel.to.line && !shiftSelecting) updateInput = false;
            ...
            if (changed) {
                var start = 0,
                    ...
                    len = Math.min(end, editing.text.length);
                var c, line = editing.from,
                    nl = -1;
                ...
                var ch = nl > -1 ? start - nl : start,
                    endline = editing.to - 1,
                    edend = editing.text.length;
                for (;;) {
        ...
        function prepareInput() {
            ...
            var from = Math.max(0, sel.from.line - 1),
                to = Math.min(lines.length, sel.to.line + 2);
        ...
        function updateDisplay(changes) {
            ...
            for (var i = 0, l = changes.length || 0; i < l; ++i) {
                ...
                for (var j = 0, l2 = intact.length; j < l2; ++j) {
                    var range = intact[j];
                    if (change.to <= range.from) intact2.push({
                        from: range.from + diff,
                        to: range.to + diff,
                        domStart: range.domStart
                    });
                    else if (range.to <= change.from) intact2.push(range);
                    else {
            ...
            if (changedLines > (visible.to - visible.from) * .3) refreshDisplay(from = Math.max(visible.from - 10, 0), to = Math.min(visible.to + 7, lines.length));
            // Otherwise, only update the stuff that needs updating.
            else patchDisplay(updates);
            ...
            if (lineDiv.childNodes.length != showingTo - showingFrom) throw new Error("BAD PATCH! " + JSON.stringify(updates) + " size=" + (showingTo - showingFrom) + " nodes=" + lineDiv.childNodes.length);
            updateCursor();
        }
        ...
        function refreshDisplay(from, to) {
            var html = [],
                ...
                inSel = posLess(sel.from, start) && !posLess(sel.to, start);
        ...
        function patchDisplay(updates) {
            ...
            for (var i = 0, e = updates.length; i < e; ++i) {
                ...
                if (badInnerHTML) for (var j = Math.max(-extra, rec.domSize); j > 0; --j) lineDiv.removeChild(nodeAfter ? nodeAfter.previousSibling : lineDiv.lastChild);
                else if (extra) {
                ...
                var node = lineDiv.childNodes[rec.domStart + off],
                    inSel = sfrom < rec.from && sto >= rec.from;
        ...
        function setSelection(from, to, oldFrom, oldTo) {
            ...
            if (posEq(from, to)) {
                if (!posEq(sel.from, sel.to)) changes.push({
                    from: oldFrom,
                    to: oldTo + 1
                });
            }
                ...
                if (!posEq(from, sel.from)) {
                    if (from.line < oldFrom) changes.push({
                        from: from.line,
        ...
        function clipPos(pos) {
            ...
            var ch = pos.ch,
                linelen = lines[pos.line].text.length;
        ...
        function scrollPage(down) {
            var linesPerPage = Math.floor(scroller.clientHeight / lineHeight()),
                head = sel.inverted ? sel.from : sel.to;
        ...
        function handleEnter() {
            replaceSelection("\n", "end");
            if (options.enterMode != "flat") indentLine(sel.from.line, options.enterMode == "keep" ? "prev" : "smart");
        }
        ...
        function indentLine(n, how) {
            ...
            if (!diff) {
                ...
                var indentString = "",
                    pos = 0;
                if (options.indentWithTabs) for (var i = Math.floor(indentation / tabSize); i; --i) {
                    pos += tabSize;
                    indentString += "\t";
                }
        ...
        function markText(from, to, className) {
            ...
            function add(line, from, to, className) {
                var line = lines[line],
                    mark = line.addMark(from, to, className);
                mark.line = line;
                accum.push(mark);
            }
        ...
        function localCoords(pos, inLineWrap) {
            var lh = lineHeight(),
                line = pos.line - (inLineWrap ? showingFrom : 0);
            return {
                x: charX(pos.line, pos.ch),
                y: line * lh,
                yBot: (line + 1) * lh
            };
        }
        ...
        function posFromMouse(e, liberal) {
            ...
            return clipPos({
                line: line,
                ch: charFromX(clipLine(line), x - offL.left)
            });
        }
        ...
        function onContextMenu(e) {
            ...
            if (posEq(sel.from, sel.to) || posLess(pos, sel.from) || !posLess(pos, sel.to)) setCursor(pos.line, pos.ch);

            var oldCSS = input.style.cssText;
            input.style.cssText = "position: fixed; width: 30px; height: 30px; top: " + (e.pageY() - 1) + "px; left: " + (e.pageX() - 1) + "px; z-index: 1000; background: white; " + "border-width: 0; outline: none; overflow: hidden; opacity: .05;";
        ...
        function getStateBefore(n) {
            var start = findStartLine(n),
                state = start && lines[start - 1].stateAfter;
        ...
        function endOperation() {
            ...
            // update.
            if (!leaveInputAlone && (updateInput === true || (updateInput !== false && selectionChanged))) prepareInput();
        ...
        function SearchCursor(query, pos, caseFold) {
            ...
            this.matches = function(reverse, pos) {
                if (reverse) {
                    var line = lines[pos.line].text.slice(0, pos.ch),
                    ...
                    var line = lines[pos.line].text.slice(pos.ch),
                        match = line.match(query),
                ...
                if (target.length == 1) this.matches = function(reverse, pos) {
                    var line = fold(lines[pos.line].text),
                        len = query.length,
                        match;
                    ...
                    var ln = pos.line,
                        ...
                        match = target[idx],
                        line = fold(lines[ln].text);
                    var offsetA = (reverse ? line.indexOf(match) + match.length : line.lastIndexOf(match));
                    if (reverse ? offsetA >= pos.ch || offsetA != match.length : offsetA <= pos.ch || offsetA != line.length - match.length) return;
                    for (;;) {
        ...
        SearchCursor.prototype = {
            ...
            find: function(reverse) {
                var self = this,
                    pos = clipPos(reverse ? this.pos.from : this.pos.to);
                ...
                for (;;) {
                    ...
                    if (reverse) {
                        ...
                        pos = {
                            line: pos.line - 1,
                            ch: lines[pos.line - 1].text.length
                        };
                    }
                    else {
    ...
    CodeMirror.getMode = function(options, spec) {
        ...
        if (typeof spec == "string") var mname = spec,
            config = {};
        else if (spec != null) var mname = spec.name,
            config = spec;
    ...
    Line.prototype = {
            ...
            if (mk) {
                ...
                for (var i = 0; i < mk.length; ++i) {
                    ...
                    if (mark.from >= end) del = true;
                        ...
                        if (mark.to != null) mark.to = fix(mark.to);
                    }
        ...
        getHTML: function(sfrom, sto, includePre, endAt) {
            ...
            else if (!marked && sfrom == null) for (var i = 0, ch = 0; ch < len; i += 2) {
                var str = st[i],
                    l = str.length;
    ...
    History.prototype = {
        addChange: function(start, added, old) {
            ...
            if (time - this.time > 400 || !last || last.start > start + added || last.start + last.added < start - last.added + last.old.length) this.done.push({
                start: start,
                added: added,
                old: old
            });
    ...
    if ("\n\nb".split(/\n/).length != 3) var splitLines = function(string) {
        var pos = 0,
            nl, result = [];
    ...
    else var splitLines = function(string) {
    ...
    if (window.getSelection) {
        ...
        var setSelRange = function(te, start, end) {
            if (start == end) te.setSelectionRange(start, end);
            else {
                te.setSelectionRange(start, end - 1);
                window.getSelection().modify("extend", "forward", "character");
            }
        };
        else var setSelRange = function(te, start, end) {
            try {
    ...
    // IE model. Don't ask.
        ...
        var selRange = function(te) {
            ...
            var val = te.value,
                len = val.length,
                localRange = te.createTextRange();
            localRange.moveToBookmark(range.getBookmark());
            var endRange = te.createTextRange();
            endRange.collapse(false);

            if (localRange.compareEndPoints("StartToEnd", endRange) > -1) return {
                start: len,
                end: len
            };
            ...
            if (localRange.compareEndPoints("EndToEnd", endRange) > -1) return {
                start: start,
                end: len
            };
