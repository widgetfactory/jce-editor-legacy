...
var CodeMirror = (function() {
	...
	function CodeMirror(place, options) {
		...
		if (options.cursorActivity)
			options.onCursorActivity = options.cursorActivity;
		...
		if (options.lineNumbers)
			this.lineNumbers = addLineNumberDiv(div, options.firstLineNumber);
	...
	CodeMirror.prototype = {
		...
		setTextWrapping: function(on) {
			if (on == this.options.textWrapping)
				return;
		...
		activateLineNumbers: function() {
			...
			function nonWrapping() {
				...
				clear = function() {
					onScroll();
					onResize();
					if (self.updateNumbers == update)
						self.updateNumbers = null;
				};
				update();
			}
			...
			function wrapping() {
				...
				function setNum(n, node) {
					...
					if (!lineNum)
						lineNum = scroller.appendChild(createHTMLElement("div"));
					if (styleNums)
						styleNums(lineNum, node, n);
				...
				function work() {
					if (!scroller.parentNode || scroller.parentNode != self.lineNumbers)
						return;
