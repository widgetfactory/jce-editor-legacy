...
(function() {
    ...
    tinymce.create('tinymce.plugins.PastePlugin', {
        ...
        _preProcess : function(pl, o) {
            ...
            var stripClass = ed.getParam(ed, 'paste_strip_class_attributes');
        ...
        _processStyles : function(node) {
            ...
            var s = ed.getParam('paste_retain_style_properties');
        ...
        _convertURLs : function(node) {
            ...
            each(dom.select('*:not(a)', node), function(el) {
                if (!dom.getParent(el, 'a')) {
                    each(el.childNodes, function(n) {
                        if (n && n.nodeType == 3) {
                            ...
                            if (s && (new RegExp(ex, 'g').test(s) || new RegExp(ux, 'g').test(s))) {
                                n.parentNode.innerHTML = processURL(s);
                            }
                        }
                    });
